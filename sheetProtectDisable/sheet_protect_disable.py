...
def main():
    ...
    try:
        xlsx_paths = get_file_paths(file_extension='.xlsx', directory_path=current_dir)
        for path in xlsx_paths:
            unzip_dir = unzip(zip_file_path=path, destination_folder=current_dir)
...
def unzip(*, zip_file_path: str, destination_folder: str) -> str:
    unziped_folder = None
    if zipfile.is_zipfile(zip_file_path):
        unziped_folder = os.path.join(destination_folder, get_filename_without_extension(file_path=zip_file_path))
        if not os.path.exists(unziped_folder):
            os.makedirs(unziped_folder)
        with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
            zip_ref.extractall(unziped_folder)
    return unziped_folder
